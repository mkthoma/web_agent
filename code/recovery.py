...
def plan_recovery(
    *,
    ...
    Decision table (all coverage):
      ...
      reason=upstream_failure, failed=planner,
        error mentions "zero valid nodes"       → replan (one recovery attempt)
    ...
    if failed_skill == "planner":
        if "zero valid nodes" in (error_text or "").lower():
            fr = (f"node={failed_node_id} skill=planner reason={reason} "
                  f"error={error_text}")
            return RecoveryDecision(
                action="replan", reason=reason,
                note="planner returned no nodes; queueing one recovery replan",
                failure_report=fr,
            )
        return RecoveryDecision(
            action="skip", reason=reason,
