"""Web Agent — growing-graph orchestrator.
...
def _configure_stdio() -> None:
    """Avoid Windows cp1252 crashes when printing box-drawing or arrows."""
...
class Graph:
    ...
    def ready_nodes(self) -> list[str]:
        # A predecessor counts as "satisfied" when it reached a terminal state.
        # `skipped` covers critic-fail and recovery skip; `failed` covers
        # upstream browser/distiller failures that will not be retried so
        # fan-in nodes (distiller → comparator) can still run with gaps.
        _SATISFIED = ("complete", "skipped", "failed")
        out = []
        for nid, d in self.g.nodes(data=True):
            if d["status"] != "pending":
                continue
            preds = list(self.g.predecessors(nid))
            if all(self.g.nodes[p]["status"] in _SATISFIED for p in preds):
                out.append(nid)
        return out
...
class Executor:
    ...
    async def run(self, query: str, *, session_id: str | None = None,
        ...
        sid = session_id or f"webagent-{uuid.uuid4().hex[:8]}"
        ...
        while True:
            ...
            for nid, result, prompt in outcomes:
                ...
                if not result.success:
                    recovery_decision = plan_recovery(
                        failed_skill=graph.g.nodes[nid]["skill"],
                        error_text=result.error or "",
                        failed_node_id=nid,
                    )
                    terminal_status = (
                        "skipped" if recovery_decision.action == "skip" else "failed"
                    )
                graph.mark(nid, terminal_status)
                ...
                if result.success:
                    ...
                    print(f"  -> {nid} skipped ({recovery_decision.reason}, "
                          f"skill={graph.g.nodes[nid]['skill']}): "
                          f"{recovery_decision.note}")
                    continue
                else:
                    assert recovery_decision is not None
        ...
        if formatter_answer is None:
            from comparison_report import resolve_final_table

            states = store.read_all_nodes()
            formatter_answer = resolve_final_table(states, graph.g, prefer_formatter=False)
