"""Web Agent skill registry + per-skill execution.
...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    # Planner on a failover provider often returns prose or empty text instead
    # of the required JSON object. Retry once on gemini before we fail the node.
    if skill.name == "planner" and not parsed.get("nodes") and not parsed.get("successors"):
        used = (reply.get("provider") or "").lower()
        if used != "gemini":
            print(f"[skills] planner: empty/unparseable JSON from {used!r}; retrying on gemini")
            reply = await asyncio.to_thread(
                LLM().chat,
                prompt=rendered,
                agent=skill.name,
    ...
    if skill.name == "planner" and not successors:
        raw_preview = (reply.get("text") or "")[:400]
        err = (
            "planner: zero valid nodes emitted (empty or unparseable JSON). "
            f"provider={reply.get('provider', '')} raw_preview={raw_preview!r}"
        )
        print(f"[skills] {err}")
        return AgentResult(
            success=False, agent_name=skill.name,
            output=parsed, successors=[],
            elapsed_s=time.time() - started,
            provider=reply.get("provider", ""),
            error=err,
        ), rendered
