...
def delete_session(session_id: str) -> bool:
    """Remove a session directory (nodes, graph, REPORT.html, browser artifacts)."""
    sid = validate_session_id(session_id)
    target = (SESSIONS_ROOT / sid).resolve()
...
def delete_all_sessions() -> int:
    """Remove every session under state/sessions/. Returns count deleted."""
    deleted = 0
