...
# Default failover ring when no tier-based auto_route is active. NVIDIA and Groq
# sit ahead of Gemini so agent-pinned workloads can spill off the 15 RPM Gemini cap.
DEFAULT_FAILOVER_ORDER = ["groq", "nvidia", "gemini", "cerebras", "github", "openrouter", "ollama"]

# V9.1 burst back-pressure. Under a parallel fan-out (comparison runs fire 5+
# browser / distiller / critic nodes in one asyncio.gather batch) the WHOLE
# failover ring can be momentarily in cooldown — every provider's 2-6s cooldown
# was started microseconds ago by a sibling request. Rather than 503 instantly,
# the chat loop waits for the soonest provider to free (bounded by this deadline)
# and retries. This is the latency mechanism honouring its own back-pressure
# instead of bailing. Only ever waits on RECOVERABLE blockers (cooldown / RPM /
# TPM / short backoff); never on hard daily caps, where waiting cannot help.
WORKER_WAIT_DEADLINE_S = float(os.getenv("GATEWAY_WORKER_WAIT_S", "30"))


def _failover_candidates(router, *, preferred: str | None = None, tier: str | None = None) -> list[str]:
    """Build an ordered provider list: preferred first, then tier/default ring."""
    if tier and tier in TIER_TO_ORDER:
        base = TIER_TO_ORDER[tier]
    else:
        base = DEFAULT_FAILOVER_ORDER
    wired = [p for p in base if p in router.providers]
    if not wired:
        wired = list(router.providers.keys())
    if preferred and preferred in router.providers:
        return [preferred] + [p for p in wired if p != preferred]
    return wired


def _min_recoverable_wait(router, candidates, est_tokens, required_caps) -> float | None:
    """Smallest seconds until SOME candidate frees, considering only
    *recoverable* blockers (cooldown / RPM / TPM / short backoff).

    Returns None when every candidate is HARD-blocked — RPD burned, daily-token
    cap hit, a required capability is missing, or the prompt exceeds max_ctx —
    because waiting cannot help those. The caller uses this to decide whether to
    back off and retry the ring or to surface 503 immediately.
    """
    now = time.time()
    ...
    for name in candidates:
        if name not in router.providers:
            continue
        limits = LIMITS[name]
        caps = getattr(router.providers[name], "capabilities", {})
        if required_caps and any(not caps.get(c) for c in required_caps):
            continue  # hard: provider lacks a needed capability
        if est_tokens > limits["max_ctx"]:
            continue  # hard: prompt too big for this provider
        st = router.state[name]
        st.gc()
        if st.calls_today >= limits["rpd"]:
            continue  # hard: daily request cap — resets at day boundary only
        if "tokens_per_day" in limits and st.tokens_today + est_tokens > limits["tokens_per_day"]:
            continue  # hard: daily token cap
        waits = [0.0]
        cd = limits["cooldown"] - (now - st.last_call)
        if cd > 0:
            waits.append(cd)
        bo = st.unavailable_until - now
        if bo > 0:
            waits.append(bo)  # short 5xx/timeout backoff; long auth/RPD bounded by deadline
        if len(st.calls_minute) >= limits["rpm"] and st.calls_minute:
            waits.append(60 - (now - st.calls_minute[0]))  # RPM window slides
        tpm = sum(t for _, t in st.tokens_minute)
        if tpm + est_tokens > limits["tpm"] and st.tokens_minute:
            waits.append(60 - (now - st.tokens_minute[0][0]))  # TPM window slides
        w = max(waits)
...
def _backoff_for(err: Exception, has_model_override: bool = False):
    ...
    if status and 500 <= status < 600: return 8, f"upstream {status}"
...
async def chat(req: ChatRequest):
    ...
    # V8: agent_routing.yaml expresses a *preference*, not a hard pin. Only an
    # explicit `provider=` on the request disables failover (explicit_override).
    ...
    if req.auto_route and not req.provider:
        ...
        candidates = _failover_candidates(
            router, preferred=agent_preferred, tier=router_decision.tier,
        )
    elif req.provider:
        candidates = router.candidates(req.provider)
    elif agent_preferred:
        candidates = _failover_candidates(router, preferred=agent_preferred)
    else:
        candidates = _failover_candidates(router)
    ...
    # V9.1: `working` is the live ring for this attempt pass; errored providers
    # are dropped from it. When it empties (all momentarily unavailable) we wait
    # for the soonest recoverable provider and refill from the full `candidates`
    # set, bounded by `_wait_deadline`. Explicit `provider=` pins fail fast.
    import asyncio as _asyncio
    working = list(candidates)
    _wait_deadline = time.time() + WORKER_WAIT_DEADLINE_S
    while True:
        name, atts = router.pick(est, working, required_caps=required_caps)
        all_attempts.extend(atts)
        if name is None:
            if explicit_override:
                break
            wait = _min_recoverable_wait(router, candidates, est, required_caps)
            if wait is None or time.time() + wait > _wait_deadline:
                break
            await _asyncio.sleep(max(0.2, min(wait + 0.05, 5.0)))
            working = list(candidates)
            continue
        ...
        try:
            ...
            working = [c for c in working if c != name]
            continue
